...
class GPT2PPL:
    ...
    def __call__(self, sentence):
        ...
        # 统计中文字符、英文字母和数字
        total_valid_char = len(re.findall(r'[\u4e00-\u9fff\w]', sentence))
        ...
        for i, line in enumerate(lines):
            # 匹配中文或字母数字
            if re.search(r'[\u4e00-\u9fff\w]+', line) is None:
                continue
            if len(offset) > 0:
                line = offset + line
                offset = ""
            ...
            if line[-1] == "\n" or line[-1] == " ":
                ...
                offset = line[-1]
                line = line[:-1]
            ppl_line = self.getPPL(line)
            Perplexity_per_line.append(ppl_line)
        
        # 处理空列表情况（如果没有有效行，使用整体困惑度）
        if not Perplexity_per_line:
            Perplexity_per_line = [ppl]
